...
echo -e "\n${YELLOW}📋 Fetching stack information...${NC}"
...
if [ -z "$FRONTEND_BUCKET" ] || [ -z "$CLOUDFRONT_ID" ]; then
    echo -e "${RED}❌ Failed to get stack outputs. Make sure the stack is deployed.${NC}"
    exit 1
fi
...
echo -e "\n${YELLOW}🔧 Creating prod config...${NC}"
...
REACT_APP_LOGOUT_URI=${WEBSITE_URL}
...
echo -e "\n${YELLOW}🏗️  Building React app...${NC}"

# Export environment variables for build
export REACT_APP_API_BASE_URL="${API_ENDPOINT}"
...
export REACT_APP_LOGOUT_URI="${WEBSITE_URL}"
